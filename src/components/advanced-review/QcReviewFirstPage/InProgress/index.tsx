...
import {
  ...
  AdvancedReviewInProgressDataAsync,
...
const InProgress: React.FC<IProps> = ({ searchQuery, label }) => {
  ...
  const [
    fetchAdvanceReviewInProgressData,
    setFetchAdvanceReviewInProgressData,
  ] = useState<IItem[]>([]);
  const [totalRecords, setTotalRecords] = useState<number>(0);
  const { advanceReviewInProgressData, status, TotalInProgressCount } =
  ...
  useEffect(() => {
    ...
    dispatch(AdvancedReviewInProgressTotalCountAsync());
    dispatch(AdvancedReviewInProgressDataAsync(payload));
  }, []);

  useEffect(() => {
    if (status === STATUS.fulfilled) {
      setTotalRecords(TotalInProgressCount);
      setFetchAdvanceReviewInProgressData(advanceReviewInProgressData);
    } else {
      setIsLoading(false);
    }
  }, [status, advanceReviewInProgressData, TotalInProgressCount]);
  ...
  useEffect(() => {
    setIsLoading(true);
    if (fetchAdvanceReviewInProgressData?.length) {
      setIsLoading(false);
      setFilteredData(fetchAdvanceReviewInProgressData);
      setMessage("");
    } else {
      setIsLoading(false);
      setFilteredData([]);
    ...
    try {
      ...
      if (StatusChangeAsync.fulfilled.match(response)) {
        if (response.payload.status === 200) {
          ...
          await dispatch(AdvancedReviewInProgressDataAsync(payload));
          const newOpenModals = [...openModals];
  ...
  const handleDownload = () => {
    ...
    const csvContent = selectedData
      .map((item) => {
        return `"${item?.monitor_id}","${item?.name}","${item?.description}","${item?.created_on}","${item?.to_date}","${item?.total_records}","${item?.pending_records}","${item?.created_by}"`;
      })
      .join("\n");
    ...
    link.setAttribute(
      ...
      }_QcInProgressList_${Utils.getCurrentDateAndTime()}.csv`
    );
  ...
  return (
    <React.Fragment>
      ...
      {AdvancedReviewInProgressDataAsync?.length > 0 && (
        <CustomPagination
