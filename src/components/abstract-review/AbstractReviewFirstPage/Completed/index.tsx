...
import {
  ...
  AbstractReviewCompletedDataAsync,
...
const Completed: React.FC<IProps> = ({ searchQuery, label }) => {
  ...
  const [
    fetchAbstractReviewCompletedData,
    setFetchAbstractReviewCompletedData,
  ] = useState<IItem[]>([]);
  const [totalRecords, setTotalRecords] = useState<number>(0);
  const { abstractReviewCompletedData, status, TotalCompletedCount } =
  ...
  useEffect(() => {
    ...
    dispatch(AbstractReviewCompletedDataAsync(payload));
  }, []);

  useEffect(() => {
    if (status === STATUS.fulfilled) {
      setTotalRecords(TotalCompletedCount);
      setFetchAbstractReviewCompletedData(abstractReviewCompletedData);
    } else {
      setIsLoading(false);
    }
  }, [status, abstractReviewCompletedData, TotalCompletedCount]);
  ...
  useEffect(() => {
    setIsLoading(true);
    if (fetchAbstractReviewCompletedData?.length) {
      setIsLoading(false);
      setFilteredData(fetchAbstractReviewCompletedData);
      setMessage("");
    } else {
      setIsLoading(false);
      setFilteredData([]);
    ...
    try {
      ...
      if (StatusChangeAsync.fulfilled.match(response)) {
        if (response.payload.status === 200) {
          ...
          await dispatch(AbstractReviewCompletedDataAsync(payload));
          const newOpenModals = [...openModals];
  ...
  const handleDownload = () => {
    ...
    link.setAttribute(
      ...
      }_AbstractCompletedList_${Utils.getCurrentDateAndTime()}.csv`
    );
  ...
  return (
    <React.Fragment>
      ...
      {fetchAbstractReviewCompletedData?.length > 0 && (
        <CustomPagination
