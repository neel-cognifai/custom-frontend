...
import {
  ...
  AbstractReviewInProgressDataAsync,
  AbstractReviewInProgressTotalCountAsync,
  StatusChangeAsync,
...
const InProgress: React.FC<IProps> = ({ searchQuery, label }) => {
  ...
  const [
    fetchAbstractReviewInProgressData,
    setFetchAbstractReviewInProgressData,
  ] = useState<IItem[]>([]);
  const [totalRecords, setTotalRecords] = useState<number>(0);
  const { abstractReviewInProgressData, status, TotalInProgressCount } =
  ...
  useEffect(() => {
    ...
    dispatch(AbstractReviewInProgressTotalCountAsync());
    dispatch(AbstractReviewInProgressDataAsync(payload));
  }, []);

  useEffect(() => {
    if (status === STATUS.fulfilled) {
      setTotalRecords(TotalInProgressCount);
      setFetchAbstractReviewInProgressData(abstractReviewInProgressData);
    } else {
      setIsLoading(false);
    }
  }, [status, abstractReviewInProgressData, TotalInProgressCount]);
  ...
  useEffect(() => {
    setIsLoading(true);
    if (fetchAbstractReviewInProgressData.length) {
      setIsLoading(false);
      setFilteredData(fetchAbstractReviewInProgressData);
      setMessage("");
    } else {
      setIsLoading(false);
      setFilteredData([]);
    ...
    try {
      ...
      if (StatusChangeAsync.fulfilled.match(response)) {
        if (response.payload.status === 200) {
          ...
          await dispatch(AbstractReviewInProgressTotalCountAsync());
          await dispatch(AbstractReviewCompletedTotalCountAsync());
          await dispatch(AbstractReviewCancelledTotalCountAsync());
          const payload = {
            pageNumber: 1,
            perPage: perPage,
          };
          await dispatch(AbstractReviewInProgressDataAsync(payload));
          const newOpenModals = [...openModals];
  ...
  const handleDownload = () => {
    ...
    const csvHeader =
      "Id,Name,Description,Date Created,Due Date,Total Records,Monitor Status,Pending Case,Created By\n";
    const csvContent = selectedData
      .map((item) => {
        return `"${item?.monitor_id}","${item?.name}","${item?.description}",${
          item?.created_on ? item?.created_on.split("T")[0] : "-"
        } ,"${item?.to_date}","${item?.total_records}","${item?.status}","${
    ...
    link.setAttribute(
      ...
      }_AbstractInProgressList_${Utils.getCurrentDateAndTime()}.csv`
    );
  ...
  return (
    <React.Fragment>
                              ...
                              {openModalIndex === index &&
                                openModals[index] && (
                                  ...
                                  >
                                    ...
                                    />
                                    <div
                                      className={`items-center text-[green] space-x-2`}
                                    >
                                      <>
                                        <div
                                          className="m-2"
                                          onClick={() =>
                                            handleStatus(
                                              item,
                                              index,
                                              CONSTANTS.MONITOR_STATUS_COMPLETED
                                            )
                                          }
                                        >
                                          Mark as Completed
                                        ...
                                        <div
                                          className="px-1 my-2 text-[red]"
                                          onClick={() =>
                                            handleStatus(
                                              item,
                                              index,
                                              CONSTANTS.MONITOR_STATUS_CANCELLED
                                            )
                                          }
                                        >
                                          Mark as Cancelled
      ...
      {fetchAbstractReviewInProgressData.length > 0 && (
        <CustomPagination
