...
const InVaildICSR: React.FC<IProps> = ({ monitor_id, label }) => {
  ...
  useEffect(() => {
    if (status === STATUS.fulfilled) {
      ...
      setTotalRecords(AbstractReviewMonitorInValidICSRTotalRecord);
    } else {
  ...
  const handleDownload = () => {
    ...
    link.setAttribute(
      ...
      `${
        monitorDetail.name
      }_AbstractInValidICSRList_${Utils.getCurrentDateAndTime()}.csv`
    );
              ...
              <tbody>
                ...
                {message && (
                  <tr className="border-b cursor-pointer text-center text-sm border-style hover:bg-ghostwhite mb-2">
                    <td
                      className="px-2 py-2 capitalize col-span-3 text-right"
                      colSpan={8}
                    >
                      {message}
                    </td>
