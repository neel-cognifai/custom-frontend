...
import { AbstractMonitorDetailsAsync, AbstractMonitorDetailsCountsAsync, AbstractReviewDataState, AbstractReviewMonitorIdAsync, AbstractReviewMonitorPotentialICSRTotalRecordIdAsync, AssignToAsync, GetTeamUserAsync, ReviewAbstractSendMailAsync } from "../../abstract-review.slice";
...
const PotentialICSR: React.FC<IProps> = ({
  monitor_id,
  label
}) => {
...
>([]);
  ...
  useEffect(() => {
    if (status === STATUS.fulfilled) {
      ...
      setTotalRecords(AbstractReviewMonitorPotentialICSRTotalRecord);
    }  else {
  ...
  const toggleRowSelection = (index: number) => {
    LocalStorage.setItem(CONSTANTS.LOCAL_STORAGE_KEYS.RecordIndex, JSON.stringify(index))
  ...
  const handleUpdatedByTagSelect = (newValues: Option[]) => {
    ...
    const filteredData =
      selectedTagValues.length === 0
        ...
        : fetchAbstractReviewMonitor?.filter((item) => {
            return selectedTagValues.some((selectedTag) =>
              item.modified_by?.toLowerCase().includes(selectedTag)
            );
          });
    setFilteredData(filteredData);
  };
  ...
  useEffect(()=> {
    LocalStorage.setItem(CONSTANTS.LOCAL_STORAGE_KEYS.DATA_STORE, JSON.stringify(sortedData))
  },[sortedData]);
  ...
  const handleDownload = () => {
    ...
    const csvContent = selectedData
      .map((item) => {
        ...
        const DesignatedMedicalEvent = item?.designated_medical_events.toString() || "-";
        const country = item?.country || "-";
        const assignee = item?.assignee || "-";
        const status = item?.status || "-";
        const modifiedBy = item?.modified_by || "-";
        const tags = item?.tags ? Object.keys(item?.tags)?.join(", "): "-";
        const medications = item?.ai_tags?.Medications ? item?.ai_tags?.Medications?.flatMap((medication) =>
          medication?.entity?.map((entity) => entity)
        ).join(", "): "-";

        return `"${item?.article_id}","${
          item?.title
        }","${tags}","${medications}","${DesignatedMedicalEvent}","${LiteratureSource}",${country},"${item?.ai_decision}","${
          item.confidence_score
        }","${item?.reason}","${item?.causality_decision}","${
          item.causality_confidence_score
        }","${
          item?.causality_reason
        }","${assignee}","${status}","${modifiedBy}"`;
      })
      .join("\n");
    ...
    link.href = url;
    link.setAttribute("download", `${monitorDetail.name}_AbstractPotentialICSRList_${Utils.getCurrentDateAndTime()}.csv`);
            ...
            <thead className="border-style text-sm text-left">
                <tr className="font-Archivo cursor-pointer header-style capitalize text-style text-sm bg-gray-50">
                  ...
                  <th className="px-4 py-2 w-40 text-right">
                    <div className="relative">
                      <span
                        className="flex hover-text-style title-width text-right items-end hover:bg-whiteGray cursor-pointer px-6 py-2"
                      >
                        Title{" "}
                      </span>
                    </div>
